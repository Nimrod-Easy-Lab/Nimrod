...
public class ComponentManager {
  ...
  public SimpleList<Sequence> getSequencesForType(StatementKind statement, int i) {
    ...
    SimpleList<Sequence> ret = null;
    if (statement instanceof RMethod && !((RMethod)statement).isStatic() && i == 0) {    	
    	ret = gralComponents.getSequencesForType(neededType, false, ((RMethod)statement).getAllowedConstructors());
//    	ret = gralComponents.getSequencesForType(neededType, false);
    }
    else
    	ret = gralComponents.getSequencesForType(neededType, false);
