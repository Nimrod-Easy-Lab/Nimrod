...
import saferefactor.localrandoop.util.ProgressDisplay;
import saferefactor.localrandoop.util.Reflection;

/**
 * An ExecutableSequence wraps a {@link Sequence} with functionality for
 * executing the sequence. It also lets the client add {@link Check}s to that
 * check expected behaviors of the execution.
 * <p>
 ...
 * <li><b>Check evaluation results.</b>Corresponding to every check is a boolean
 * value that represents whether the check passed or failed during the last
 ...
 * sequence represents. This method uses reflection to execute each element in
 * the sequence (method call, constructor call, primitive or array declaration,
 * etc). Before executing each statement (e.g. the i-th statement), execute(v)
 ...
 * example, it may decorate the sequence with {@link Check}s about the
 * execution.
 * 
 * NOTES.
 * <p>
...
public class ExecutableSequence implements Serializable {
		...
		for (int ii = 0; ii < checks.size(); ii++) {
			...
			if (false) {
				if (obs1.size() != obs2.size()) {
					if ((ii < (checks.size() - 1)) && (obs1.size() == 0)
							&& (obs2.size() == 1)) {
						System.out
								.printf("keeping mismatched check %s%n", obs2);
			...
			if (true) {
				for (int obs : diff_obs1) {
					Check check = obs1.get(obs);

					Variable variable = this.sequence.getVariable(obs);
					System.out.println("Var: " + variable);
					StatementKind declaringStatement = variable
							.getDeclaringStatement();
					System.out.println("Statement: " + declaringStatement);

					System.out.println("before:");
					System.out.println(check.get_value());
					Check comparedCheck = obs2.get(obs);
					System.out.println("afeter:");
					System.out.println(comparedCheck.get_value());
				}
			}
		}
	...
	public Set<String> compare_checks(ExecutableSequence es, boolean print) {
		...
		for (int ii = 0; ii < checks.size(); ii++) {
			...
			for (int o1i = 0; o1i < obs1.size(); o1i++) {
				...
				if (o2i >= obs2.size()) {
					diff_obs1.add(0, o1i);
					print_diffs(print, "extra obs1", ob1, null, o1i, -1, es);
				}
				boolean match = false;
				int o2i_start = o2i;
				while (o2i < obs2.size()) {
					...
					if (ob1.get_id().equals(ob2.get_id())) {
						...
						if (!ob1.get_value().equals(ob2.get_value())) {
							diff_obs1.add(0, o1i);
							diff_obs2.add(0, o2i);
							print_diffs(print, "mismatch", ob1, ob2, o1i, o2i,
									es);
						}
						match = true;
						break;
				...
				if (match) {
					for (int jj = o2i_start; jj < o2i; jj++) {
						diff_obs2.add(0, jj);
						print_diffs(print, "obs2 missing", null, obs2.get(jj),
								o1i, o2i, es);
					}
					o2i++;
				} else { // no match for obs1
					diff_obs1.add(0, o1i);
					print_diffs(print, "obs1 missing", ob1, null, o1i, -1, es);
					o2i = o2i_start;
				}
			}
			for (; o2i < obs2.size(); o2i++) {
				diff_obs2.add(0, o2i);
				print_diffs(print, "extra obs2", null, obs2.get(o2i), -1, o2i,
						es);
			}

			// for each diferent result, get the method call related to that
			for (int obs : diff_obs1) {
				Check check = obs1.get(obs);

				if (check instanceof ObjectCheck) {
					ObjectCheck objectCheck = (ObjectCheck) check;
					Variable[] vars = objectCheck.vars;
...
//								+ declaringStatement);
						...
						if (declaringStatement instanceof RMethod)
							result.add(declaringStatement.toString());
