...
import saferefactor.localrandoop.util.PrimitiveTypes;
...
import saferefactor.localrandoop.util.Reflection.Match;
...
public class ForwardGenerator extends AbstractGenerator {
	...
	public void processSequence(ExecutableSequence seq) {
		...
		if (GenInputsAbstract.offline) {
			if (Log.isLoggingOn()) {
				Log
						.logLine("Making all indices active (offline generation specified; sequences are not executed).");
			}
		...
		if (seq.hasNonExecutedStatements()) {
			if (Log.isLoggingOn()) {
				Log
						.logLine("Making all indices inactive (sequence has non-executed statements, so judging it inadequate for further extension).");
			}
		...
		if (seq.hasFailure()) {
			if (Log.isLoggingOn()) {
				Log
						.logLine("Making all indices inactive (sequence reveals a failure, so judging it inadequate for further extension)");
			}
			seq.sequence.clearAllActiveFlags();
			return;
		}
		...
		if (!seq.isNormalExecution()) {
			if (Log.isLoggingOn()) {
				Log
						.logLine("Making all indices inactive (exception thrown, or failure revealed during execution).");
			}
			seq.sequence.clearAllActiveFlags();
			return;
		}
		...
		for (int i = 0; i < inputTypes.size(); i++) {
			...
			if (GenInputsAbstract.alias_ratio != 0
				...
				for (Class<?> match : types.getMatches(t)) {
					...
					// GUSTAVO: verifica se o construtor eh permitido para o
					// method de acordo com a analise do saferefactor
					if (isReceiver) {
						if (((RMethod) statement).getAllowedConstructors()
								.contains(match.toString()))
							candidateVars.add(new ArrayListSimpleList<Integer>(
			...
			if (GenInputsAbstract.always_use_ints_as_objects
				...
				if (Log.isLoggingOn())
					Log
							.logLine("Integer-as-object heuristic: will use random Integer.");
				...
				if (Log.isLoggingOn())
					Log
							.logLine("Array creation heuristic: will create helper array of type "
									+ t);
			...
			if (l.size() == 0) {
				if (isReceiver || GenInputsAbstract.forbid_null) {
					if (Log.isLoggingOn())
						Log
								.logLine("forbid-null option is true. Failed to create new sequence.");
					return new InputsAndSuccessFlag(false, null, null);
				} else {
					if (Log.isLoggingOn())
			...
			if (!isReceiver && GenInputsAbstract.null_ratio != 0
				...
				if (Log.isLoggingOn())
					Log
							.logLine("null-ratio option given. Randomly decided to use null as input.");
				StatementKind st = PrimitiveOrStringOrNullDecl
						.nullOrZeroDecl(t);
				Sequence seq = new Sequence().extend(st,
						new ArrayList<Variable>());
			...
			Variable randomVariable = chosenSeq
					.randomVariableForTypeLastStatement(t, m);
