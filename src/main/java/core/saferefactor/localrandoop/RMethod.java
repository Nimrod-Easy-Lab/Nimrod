...
import saferefactor.localrandoop.util.MethodReflectionCode;
...
public final class RMethod implements StatementKind, Serializable {
	...
	private List<String> allowedConstructors;
	...
	private RMethod(Method method, List<String> allowedConstructors) {
		if (method == null)
			throw new IllegalArgumentException("method should not be null.");

		this.method = method;
		this.method.setAccessible(true);
		this.setAllowedConstructors(allowedConstructors);
	}
	...
	private static StatementKind getRMethod(Method method,
			List<String> allowedConstructors) {

		return new RMethod(method, allowedConstructors);
	}
	...
	public static StatementKind parse(String s) {
		...
		String descr = s;
		if (colonIdx != -1) {
			descr = s.substring(0, colonIdx).trim();
			String constructors = s.substring(colonIdx + 1).trim();

			String[] listConstructors = constructors.split(";");
			List<String> allowedConstructors = new ArrayList<String>(
					listConstructors.length);
			// TODO fix bug: randoop is trying to instantiate abstract classes
			// by doing that
			for (String constructor : listConstructors) {
				allowedConstructors.add(constructor);
			}

			return RMethod.getRMethod(Reflection.getMethodForSignature(descr),
					allowedConstructors);
		}
	...
	public void setAllowedConstructors(List<String> allowedConstructors) {
		this.allowedConstructors = allowedConstructors;
	}

	public List<String> getAllowedConstructors() {
